...
const Items = {
  all: (page) => requests.get(`/items?${limit(20, page)}`),
  bySeller: (seller, page) =>
    requests.get(`/items?seller=${encode(seller)}&${limit(20, page)}`),
  byTag: (tag, page) =>
    requests.get(`/items?tag=${encode(tag)}&${limit(20, page)}`),
  del: (slug) => requests.del(`/items/${slug}`),
  favorite: (slug) => requests.post(`/items/${slug}/favorite`),
  favoritedBy: (seller, page) =>
    requests.get(`/items?favorited=${encode(seller)}&${limit(20, page)}`),
